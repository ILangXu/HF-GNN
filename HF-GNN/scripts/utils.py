...
但是如果你使用的是PyTorch等框架，还要看一下框架的种子是否固定了。还有，如果你用了cuda，别忘了cuda的随机数种子。
这里还需要用到torch.backends.cudnn.deterministic.
'''
...
# def pocket_truncate(protein_file, ligand_file, pocket_out_file, complex_out_file, distance=5, sanitize=True): #生成蛋白质和配体的复合物文件
#     ligand = Chem.MolFromMolFile(ligand_file, sanitize=sanitize)#可以读取mol也可以读取sdf
#     structure = parsePDB(protein_file)#解析蛋白质pdb的结构
#     if ligand and structure:
#         protein = structure.select('protein')  # remove water and other useless
#         selected = protein.select('same residue as within %s of ligand' % distance, ligand=ligand.GetConformer().GetPositions())#按照距离阈值筛选蛋白质
...
#                 pickle.dump([ligand, pocket], f) #序列化，保存成文件
#         else:
...
def pocket_truncate(protein_file, ligand_file, complex_out_file, sanitize=True): #生成蛋白质和配体的复合物文件
    ligand = Chem.MolFromMolFile(ligand_file, sanitize=sanitize)#可以读取mol也可以读取sdf
    #structure = parsePDB(protein_file)#解析蛋白质pdb的结构
    pokcet = Chem.MolFromPDBFile(protein_file,sanitize=sanitize)
    if ligand and pokcet:
        # not contain H
        with open(complex_out_file, 'wb') as f:
                pickle.dump([ligand, pokcet], f) #序列化，保存成文件

    elif ligand is None and pokcet is not None:
        print('only ligand file read error for %s' % ligand_file)
    elif pokcet is None and ligand is not None:
        print('only protein file read error for %s' % protein_file)
    else:
        print('both protein file and ligand file read error for %s' % protein_file)