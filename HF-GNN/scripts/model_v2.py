...
class EdgeWeightAndSum_V2(nn.Module):
    ...
    """
    def __init__(self, in_feats): #outdim_g3
...
class HFGNN(nn.Module):
    def __init__(self, node_feat_size, edge_feat_size, num_layers, graph_feat_size, outdim_g3,
        ...
        self.hyper_conv = HypergraphConv(node_feat_size,256,dropout=0.4)
    ...
    def forward(self, hyper_bg, bg3):
        a2a = ('atom', 'interacts', 'atom')
        a2f = ('atom', 'to', 'func_group')
        f2f = ('func_group', 'interacts', 'func_group')
        #bg = hyper_bg[a2a]
        atom_feats = hyper_bg[a2a].ndata.pop('h')
        bond_feats = hyper_bg[a2a].edata.pop('e')
        g_index_u, g_index_v = hyper_bg[a2a].edges()
        input_index = torch.stack((g_index_u, g_index_v), dim=0)
        atom_feats = self.cov_graph(hyper_bg[a2a], atom_feats, bond_feats)
        atom_feats_h = self.hyper_conv(atom_feats, input_index)
        atom_feats_h = F.relu(atom_feats_h)
        atom_feats = torch.cat([atom_feats,atom_feats_h],dim=-1)
        atom_feats = self.encoder(atom_feats)
        atom_feats = F.relu(atom_feats)
