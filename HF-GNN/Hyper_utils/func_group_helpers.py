...
#生成各种原子特征
ATOM_FEATURIZER = BaseAtomFeaturizer({'atom_type': atom_type_one_hot,#原子类型
                                      'atomic_number': atomic_number, #原子序数
                                      'atom_mass': atom_mass,#原子量
                                      'atom_degree': atom_degree_one_hot, #原子度
...
BOND_FEATURIZER = BaseBondFeaturizer({'bond_type': bond_type_one_hot,
                                      'in_ring': bond_is_in_ring, #了解键是否在任何大小的环中。
                                      'conj_bond': bond_is_conjugated_one_hot})#键是否共轭的一个热编码
...
磷=[]
    ...
    return carbons, nitrogens, sulfurs, oxygens, phosphorus#(C,N,S,O,P)
...
def cyclic_atoms(G, edge_name=None):
    ...
    for i in range(len(G)):#遍历每个分子
        cycle_list_i = []  # nodes in cycle for each molecule
        edges_in_cycle_i = []
        '''
        返回构成G循环基础的循环列表。
        网络循环的基础是循环的最小集合，使得网络中的任何循环都可以写成基础中循环的总和。这里，循环的总和被定义为边的“异或”。循环基是有用的，例如，当使用基尔霍夫定律推导电路方程时。
        参数：
        GNetworkX图形
        rootnode，可选
        指定基础的起始节点。
...
def func_groups(G):
    ...
    phosphorus_grps = []
    (carbons, nitrogens, sulfurs, oxygens, phosphorus) = atom_groups(G)#获取所有原子
    (noncyc_carbons, noncyc_nitrogens, noncyc_sulfurs, noncyc_oxygens, noncyc_phosphorus) = noncyc_atoms(G)#获取不再环中的原子
    ...
    for i in range(len(G)):
        ...
        "C as the central atom 以碳原子为中心"
        for node_i in carbons[i]:
            neighbors_i = list(G[i].neighbors(node_i))  # neighbors of central atom 获取当前分子当前原子的邻居原子

            # for molecules with C=O
            # "C double bonded to O 判断当前节点和当前邻居节点之间的化学键类型是否双键且邻居节点是氧原子，若当前邻居中存在这样的，就继续往下"
            if (True in (((list(G[i][node_i][neighbor_i]['bond_type']) == double_bond) and (
                ...
                if len([neighbor_i for neighbor_i in neighbors_i if
                    ...
                    if (([neighbor_i for neighbor_i in neighbors_i if
                        ...
                        nlist = list(dict.fromkeys([node_i] + neighbors_i + all_neighbors_i))#fromkeys()方法也是返回一个新创建的字典对象，但是这个字典非常特殊， value默认为None。
...
def edges_in_func_groups(G):
    ...
    for i in range(len(G)):
        ...
        for alkene_idx, nodes_in_alkene in enumerate(alkene[i]):
            pairs = list(itertools.combinations(nodes_in_alkene, 2))#返回列表中所有可能的两两组合
            for (u, v) in pairs:
...
def get_incidence_matrix(G, use_cycle):
    A = []
    (carbons, nitrogens, sulfurs, oxygens, phosphorus) = atom_groups(G)#(C,N,S,O,P)
    (cycle_list, edges_in_cycle, nodes_in_cycle) = cyclic_atoms(G)#环状原子,获取环的列表，环中的边，环中的节点
    (carboxyl, carbamate, carbamide, amide, ketone, aldehyde, ketene, isocynate, isothiocynate, thione, thioamide,
     thiourea, oxime, hydrazone, carboximidamide, imine, carbodiimide, alcohol, thiol, allene, alkyne, ether, peroxide,
     thioether, sulfoxide, sulfone, sulfonamide, sulfonate, disulfide, amine, nitrile, hydrazine, azo, c_nitroso,
     hydroamine, nitro, phosphorus_grps, alkene) = func_groups(G)
    ...
    for i in range(len(G)):
        node_list_i = list(G[i].nodes())#获取图的节点
        edge_list_i = list(G[i].edges())#获取图的边

        all_groups_i = (alkene[i] + #把所有官能团信息加起来
                        alkyne[i] +
                        allene[i] +
                        carboxyl[i] +
                        ketene[i] +
        ...
        edges_in_groups_i = (edges_in_alkene[i] + #把官能团中的边信息相加
                             edges_in_alkyne[i] +
                             edges_in_allene[i] +
                             edges_in_carboxyl[i] +
                             edges_in_ketene[i] +
        ...
        num_grps_i = len(all_groups_i)#官能团个数

        edges_in_groups_i.sort()
        edges_in_groups_i = list(edges_in_groups_i for edges_in_groups_i, _ in itertools.groupby(edges_in_groups_i))#把可迭代对象中相邻的重复元素挑出来放一起，这里去重
        res = [list(ele) for ele in edge_list_i]
        edges_not_grouped_i = [x for x in res if x not in edges_in_groups_i]#找出不在官能团中的边
        num_edges_not_grouped_i = len(edges_not_grouped_i)

        num_hyedges_i = num_grps_i + num_edges_not_grouped_i #超边数目
        ...
        #构造超图的邻接矩阵，行是节点，列是超边，判断超边的数目不为0
        if num_hyedges_i != 0:
            A_i = np.zeros((num_nodes_i, num_hyedges_i))
            A_i = pd.DataFrame(A_i, columns=hyedge_list_i, index=row_index_i)

            # node index
            node_index_i = dict((node, i) for i, node in enumerate(node_list_i))#给节点一个索引

            # indices
            for ei, e in enumerate(edge_list_i): #遍历边中的节点
                (u, v) = e[:2]
                ...
                for k in range(len(all_groups_i)):
                    if set([u, v]).issubset(set(all_groups_i[k])) == True: #用于判断集合的所有元素是否都包含在指定集合中
                        subset = (u, v)
                        A_i.iloc[ui, k] = 1 #行ui，列k的数据
                        A_i.iloc[vi, k] = 1

            p = len(all_groups_i)
            q = len(hyedge_list_i)

            for i, (ri, r) in zip(range(p, q), enumerate(edges_not_grouped_i)): #把非官能团的超边添加上
                (u, v) = r[:2]
...
def get_graph(G, use_cycle):#false
    a = A_to_np(G, use_cycle) #超图邻接矩阵，行是节点，列是超边
    g = []
    for i in range(len(G)):
        row_i, column_i = a[i].shape
        col_row_i = [] #把所有邻接矩阵中为1的行列标挑出来，第一个值是列，也就是超边的索引，第二个是节点索引
        for j in range(row_i):
            for k in range(column_i):
                if a[i][j, k] == 1:
                    col_row_i.append([k, j])#注意，这里k和j掉包了，列是节点，行是超边
                col_row_i.sort()

        nodes_i = []
        hyedges_i = []
        for i in range(len(col_row_i)):
            hyedges_i.append(col_row_i[i][0])#超边索引的集合
            nodes_i.append(col_row_i[i][1])#节点索引的集合

            g_i = [nodes_i, hyedges_i]
            g_i = torch.LongTensor(g_i)
        g.append(g_i)
    return g #返回的是一个tensor，有两列，每一列都是一个list，0是节点的索引，1是对应的超边的索引，维度是[1,1,2,在超边中的节点数]